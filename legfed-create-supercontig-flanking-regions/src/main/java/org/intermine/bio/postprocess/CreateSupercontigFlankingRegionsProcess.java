 ...
 *
 */
public class CreateSupercontigFlankingRegionsProcess extends PostProcessor {

    private static final Logger LOG = Logger.getLogger(CreateSupercontigFlankingRegionsProcess.class);
    ...
    /**
     * Create a new CreateSupercontigFlankingRegionsProcess object that will operate on the given
     * ObjectStoreWriter.
     *
     * @param osw the ObjectStoreWriter to use when creating/changing objects
     */
    public CreateSupercontigFlankingRegionsProcess(ObjectStoreWriter osw) {
        super(osw);
    ...
    private void createAndStoreFlankingRegion(Supercontig chr, Location geneLoc, Gene gene) throws ObjectStoreException {
        ...
        if (chr.getLength() == null) {
            LOG.warn("Attempted to create GeneFlankingRegionsProcess on a supercontig without a length: " + chr.getPrimaryIdentifier());
            return;
        }
